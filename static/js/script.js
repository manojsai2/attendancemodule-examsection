...
const signHodButton = document.getElementById('signhod');
...
signUpButton.addEventListener('click', () => {
  ...
  if (Student.style.visibility === 'hidden') {
...
signHodButton.addEventListener('click', () => {
  container.classList.remove('right-panel-active');
	container.classList.add('left-panel-active');
...
// Set the desired date for disabling the button
var disabledate = document.getElementById("date");
// Check the date periodically and disable the button if necessary
setInterval(function() {
  var currentDate = new Date();
  if (currentDate > disabledate) {
    myButton.disabled = true;
  }
}, 1000); // check the date every second
